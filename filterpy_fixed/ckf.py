...
class CubatureKalmanFilter(object):
    ...
    def predict(self, dt=None, fx_args=()):
        ...
        if not isinstance(fx_args, tuple):
            fx_args = (fx_args,)
        # print(1, self.x.shape, self.P.shape)
        ...
        # print(2, self.x.shape, self.P.shape)
        # save prior
        self.x_prior = self.x.copy()
        self.P_prior = self.P.copy()
    ...
    def update(self, z, R=None, hx_args=()):
        ...
        zp = np.ravel(zp)
        # print(z.shape, np.ravel(zp).shape)
        self.y = self.residual_z(z, np.ravel(zp))  # residual

        # print("dot=", dot(self.K, self.y).shape)
        # print("self.K=", self.K)

        # print("self.y prev=", self.y)
        # self.y = self.y[0]
        # print("self.y=", self.y)
